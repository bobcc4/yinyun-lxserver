...
- 恢复同一音云服务端内部用户之间的歌单分享功能。
- 跨服务端分享和 JSON 导出合并为一个按钮，点击后选择具体操作。
- 新增管理员配置项“公网访问地址”；未配置时生成跨服务端链接会明确提示先配置，JSON 导出不受影响。
- 跨服务端分享链接不再根据当前请求地址自动猜测，避免反向代理或内网地址生成不可访问的链接。
...
- 修复外部音乐库请求封面时错误访问 `cache` 索引，导致 `External music libraries only support the music folder` 和 `Unhandled Rejection` 的问题。
- 封面请求异常现在会记录警告并返回 404，不再让未处理的异步异常影响播放或切歌。
- 新增“外部音乐”分类，显示当前账户配置的外部音乐库歌曲，支持搜索、筛选、排序、分页、播放、加入歌单和保存到设备。
- 外部音乐库按只读目录处理，隐藏并阻断删除、移动、洗版、歌词写入、元数据修改和索引关联等操作。
- 管理后台新增“扫描已挂载目录”：可发现 `/server/external/<用户名>/<库名称>` 下的挂载目录，管理员确认后自动登记并扫描，避免手动填写库名。
- 外部音乐页面在没有可用外部音乐库时提示“请联系管理员在后台管理新增外部音乐库”。

## v1.6.7 (2026-09-12)

### 安全与依赖维护

- 合并 Dependabot 安全更新：`fast-uri` 3.1.7、`@xmldom/xmldom` 0.8.15、`sharp` 0.35.4。

### Web 播放器改进

- 系统日志界面适配主题色和深浅外观，日志内容在不同主题下保持清晰可读。
- 本地音乐的格式、音质、来源、状态、排序和目录筛选条件按服务器与登录账户隔离保存。
- 优化移动端设置页面的滑动条、下拉框、字体选择器、提示信息和底部安全间距。
- 腾讯音乐和酷狗音乐在平台支持的接口中优先请求更高分辨率的封面与歌手图片。
- 清理已停用的 `PLAYER_PATH` 前端配置残留，避免自定义旧配置导致 PWA 路径错误。
- 修复 `LIST_ADD_MUSIC_LOCATION_TYPE=bottom` 在 Web 播放器中不生效的问题。
- 自定义下拉菜单正确过滤交互：被标记为 `disabled` 的平台选项不再可以点击选择。
- 本地音乐、歌单和排行榜分页增加首页与末页快捷按钮。
- Web 播放器与服务端自动换源顺序统一为 QQ、网易云、酷我、酷狗、咪咕（`tx → wy → kw → kg → mg`）。
- 将“仅下载模式”的界面名称调整为“下载并保存到本地”，明确歌曲会写入服务端持久化音乐目录。

### 升级说明

- 本版本不修改数据目录、数据库或 `/api/v1` 接口结构，可从 `v1.6.6` 直接升级。

## v1.6.6 (2026-09-02)

### 元信息补全

- 修复本地音乐“补全元信息”未将 `Album Artist` 写入音频标签的问题。
- 补全后重新读取音频文件即可获得正确的专辑艺术家信息；未提供专辑艺术家时回退到歌曲歌手。
