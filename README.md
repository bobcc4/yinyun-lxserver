...
<div align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/github/v/release/bobcc4/yinyun-lxserver?style=flat-square&color=blue" alt="Version">
...
Web 播放器保留两种歌单分享方式：分享给同一服务端的其他用户，或跨服务端分享。跨服务端分享按钮会让你选择生成带有效期的链接或导出 JSON；另一台音云服务器登录后可粘贴链接或导入文件，在确认导入预览后创建普通歌单。生成跨服务端链接前，需要在管理后台“系统配置”中填写公网访问地址；未配置时仍可导出 JSON。接收方本地已有的歌曲仍会保留在歌单中，播放时优先使用本地文件；分享包只包含歌曲识别信息，不包含音频文件、密码、Token、代理或临时播放地址。

**洛雪歌单导入（v1.7.4 起）：** 在 **设置 → 歌单分享 → 导入歌单文件** 中选择洛雪导出的 `.lxmc` 或歌单 `.json`，预览后勾选需要的歌单并确认导入。支持单歌单、多歌单以及完整备份中的歌单；不覆盖现有歌单，不恢复洛雪设置或音源，不传输音频文件。原电脑本地文件条目会提示并跳过；在线播放仍需要音云配置可用音源。这是单向文件导入，不是官方客户端实时同步，也不表示音云分享链接可以直接导入官方洛雪客户端。详见[歌单分享与导入帮助](https://bobcc4.github.io/yinyun-lxserver/guide/sharing)。
